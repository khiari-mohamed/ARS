...
// Run this script from the server directory or adjust the path
const path = require('path');
const serverPath = path.join(__dirname, 'server');

// Try to load Prisma from server directory
let PrismaClient;
try {
  PrismaClient = require(path.join(serverPath, 'node_modules', '@prisma', 'client')).PrismaClient;
} catch (error) {
  ...
  console.error('  node ../fix-sla-data.js');
  console.error('\nOr install dependencies in server directory:');
