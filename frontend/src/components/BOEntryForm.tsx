// src/components/BOEntryForm.tsx
...
import { AutoAwesome, Refresh, SmartToy } from '@mui/icons-material';
import { fetchClients } from '../services/clientService';
import { fetchContractsByClient } from '../services/contractService';
import { createBOEntry, classifyDocument } from '../services/boService';
...
const sx = {
  ...
  generateBtn: {
    ...
    px: 2,
    '&:hover': {
      background: 'linear-gradient(135deg, #163050 0%, #1d4f8c 100%)',
    },
    '&:disabled': {
      background: '#cfd8dc',
      color: '#fff',
  ...
  refreshBtn: {
    ...
    color: '#1e3a5f',
    '&:hover': { background: '#f0f4ff', borderColor: '#2563a8' },
  },
  dialogActions: {
    px: 3,
    py: 2,
...
const BOEntryForm: React.FC<Props> = ({ open, onClose, onSuccess }) => {
  ...
  // Track whether reference was user-typed (to avoid overwriting it)
  ...
  const handleGenerateReference = async () => {
    ...
    try {
      ...
      // Classify the newly generated reference
      try {
        setClassifying(true);
        ...
        // Non-fatal — classification is best-effort
      } finally {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const result = await createBOEntry({ ...formData, startTime: Date.now() });

      if (result?.success || result?.bordereau) {
        try {
          ...
          // Document creation is secondary — log but don't block
          console.warn('Document creation failed (non-fatal)');
        }
        onSuccess();
        onClose();
      } else {
        setError(result?.error ?? "Erreur lors de la création de l'entrée");
      }
    } catch (err: any) {
  ...
  // ── Selected client object for Autocomplete ───────────────────────────────
        ...
        {classification && (
          <Box sx={sx.classificationBox}>
            <Box display="flex" alignItems="center" gap={1} mb={1.5}>
              <SmartToy sx={{ fontSize: 16, color: '#0d47a1' }} />
              <Typography
                sx={{ fontWeight: 700, color: '#0d47a1', fontSize: '0.78rem', textTransform: 'uppercase', letterSpacing: 1 }}
              >
                Classification AI
              </Typography>
            </Box>
