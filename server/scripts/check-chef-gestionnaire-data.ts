...
async function checkChefGestionnaireData() {
  ...
  try {
    ...
    console.log(`   Username: ${chef1.username}`);
    ...
    gestionnaires.forEach((gest, idx) => {
      console.log(`   ${idx + 1}. ${gest.fullName} (${gest.username}) - ID: ${gest.id}`);
    });
    console.log('');
    ...
    for (const gestionnaire of gestionnaires) {
      ...
      console.log(`\n👤 ${gestionnaire.fullName} (${gestionnaire.username})`);
