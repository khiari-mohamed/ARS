...
async function checkRetourneData() {
  ...
  try {
    ...
    if (retourneCount === 0) {
      ...
      // Create a test RETOURNE bordereau
      console.log('\n🔧 CREATING TEST RETOURNE BORDEREAU...');
      const testUser = await prisma.user.findFirst({
        where: { role: 'GESTIONNAIRE' }
      });
      
      if (testUser) {
        const testBordereau = await prisma.bordereau.create({
          data: {
            reference: `TEST-RETOURNE-${Date.now()}`,
            statut: 'RETOURNE',
            dateReception: new Date(),
            nombreBS: 5,
            delaiReglement: 30,
            assignedToUserId: testUser.id,
            currentHandlerId: testUser.id,
            clientId: 'test-client-id'
          }
        });
        console.log(`  ✅ Created test RETOURNE bordereau: ${testBordereau.reference}`);
      }
    } else {
